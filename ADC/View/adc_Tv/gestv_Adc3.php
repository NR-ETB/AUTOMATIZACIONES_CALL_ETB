...
<?php include('../components/dashboard_gen.php'); ?>
    ...
    $sql = "SELECT meg_Tv FROM TELEVISION WHERE id_Tv = 2012";
    $result = $conn->query($sql);
    ...
    if ($result->num_rows > 0) {
        ...
        $pregunta_decima = $row['meg_Tv'];
    } else {
        $pregunta_decima = "No se encontró la pregunta."; // Mensaje por defecto si no se encuentra la pregunta
    }
    ...
    <div class="cards" id="quest3">
        ...
        <div class='body-card'>
            <div class='question_1'>
                <textarea id='que2' placeholder='Escribe aquí la pregunta' disabled><?php echo htmlspecialchars($pregunta_decima); ?></textarea>
            </div>
        </div>
        ...
        $sql = "SELECT id_ResTv, pre_ResTv FROM RESPUESTA_TV WHERE id_ResTv IN (1006)";
        $result = $conn->query($sql);
        ?>

        <script>
        function sendFormData() {
            // Prevenir el envío del formulario para evitar recarga de la página
            event.preventDefault();
            
            // Verificar los valores antes de enviar
            console.log("Antes de enviar el formulario:");
            console.log("Valor del select: ", $('#respuesta').val());
            console.log("Valor del textarea: ", $('#respuestaTexto').val());

            // Obtener los datos del formulario
            var formData = $('#myForm').serialize();
            
            // Imprimir los datos del formulario serializados para depuración
            console.log("Datos serializados: ", formData);

            // Enviar datos mediante AJAX (sin recargar la página)
            $.ajax({
                type: 'GET',
                url: '',  // La misma URL para enviar el formulario
                data: formData,
                ...
                complete: function() {
                    // Depuración después del envío
                    console.log("Después de enviar el formulario:");
                    console.log("Valor del select (después del envío): ", $('#respuesta').val());
                    console.log("Valor del textarea (después del envío): ", $('#respuestaTexto').val());
        ...
        <button onclick="ques3(),empty(),sendFormData();">Siguiente >></button>
    ...
    // Consulta para obtener la sexta pregunta (id_Tv = 6, por ejemplo)
    $sql = "SELECT meg_Tv FROM TELEVISION WHERE id_Tv = 2010";
    $result = $conn->query($sql);
    ...
    if ($result->num_rows > 0) {
        // Obtener la sexta pregunta
        $row = $result->fetch_assoc();
        $pregunta_once = $row['meg_Tv'];
    } else {
        $pregunta_once = "No se encontró la pregunta."; // Mensaje por defecto si no se encuentra la pregunta
    }
    ...
    <div class="cards" id="quest4" style="display: none;">
        ...
        <div class='body-card'>
            <div class='question_1'>
                <textarea id='que2' placeholder='Escribe aquí la pregunta' disabled><?php echo htmlspecialchars($pregunta_once); ?></textarea>
            </div>
        </div>
        ...
        // Consulta SQL para obtener las opciones del select (Solo los id_ResTv y pre_ResTv)
        $sql = "SELECT id_ResTv, pre_ResTv FROM RESPUESTA_TV WHERE id_ResTv IN (1003, 1011)";
        $result = $conn->query($sql);
        ?>
    ...
    <div class="cards" id="quest5" style="display: none;">
        ...
        <div class='body-card'>
            <div class='question_1'>
                <textarea id='que2' placeholder='Escribe aquí la pregunta' disabled><?php echo htmlspecialchars($pregunta_septima); ?></textarea>
            </div>
        </div>
        ...
        <button onclick="ques5();">Siguiente >></button>
